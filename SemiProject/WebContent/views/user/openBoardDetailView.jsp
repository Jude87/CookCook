						  ...
						  <div class="modal-dialog" role="document">
						    <div class="modal-content">
						      <div class="modal-header">
						        <h5 class="modal-title" id="exampleModalLabel">댓글 수정 내용 입력</h5>
						        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
						          <span aria-hidden="true">&times;</span>
						        </button>
						      </div>
						      <div class="modal-body">
						      	<textarea id="updateContent" rows="2" cols="50"
													style="resize: none;"></textarea>
						      </div>
						      <div class="modal-footer">
						        <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">취소</button>
						        <button type="button" class="btn btn-outline-secondary"  data-dismiss="modal" id="mdclose" onclick="UpdateReply();">수정하기</button>
						        </div>
						      </div>
						    </div>
						  </div>
			...
			<script>
				...
				function insertComments(ono) {
					...
					$.ajax({
						url : "rinsert.op",
						data : {
							ono :<%=b.getBoardNo()%>, // 댓글 작성한 게시글의 번호
							content : $("#replytext").val() // 댓글 작성한 글 내용
						}, 
						type : "post",
				...
				//댓글 수정버튼을 클릭!했을때 댓글의 번호를 전역변수 reUpdateNo에 담아주자
				$(document).on("click","table #coUpdateNo",function(){
					coUpdateNo = $(this).val();	
				});
				...
				function UpdateReply(){
					$("#replyNo.btn btn-outline-secondary").on('click',function(){
